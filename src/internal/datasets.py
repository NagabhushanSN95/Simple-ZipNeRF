...
from internal.utils import DataSplit
...
class NeRFSceneManager():
    """
    Loads images and poses from the dataset
    Modified from the original one to not use pycolmap
    """

    def __init__(self, data_dirpath, scene_name, resolution_suffix):
        self.data_dirpath = data_dirpath
        self.scene_name = scene_name
        self.resolution_suffix = resolution_suffix
        self.camera_type = 'SIMPLE_PINHOLE'  # TODO SNB: this may need to be loaded from dataset for different types
        return
    ...
    def process(self):
        ...
        Returns:
          a tuple [frame_names, poses, pixtocam, distortion_params].
          frame_names:  contains the only the basename of the images.
          poses: [N, 4, 4] array containing the camera to world matrices.
          pixtocam: [N, 3, 3] array containing the camera to pixel space matrices.
          distortion_params: mapping of distortion param name to distortion
            parameters. Cameras share intrinsics. Valid keys are k1, k2, p1 and p2.
        """
        scene_dirpath = self.data_dirpath / f'all/database_data/{self.scene_name}'
        rgb_dirpath = scene_dirpath / f'rgb{self.resolution_suffix}'
        frame_names = sorted([path.stem for path in sorted(rgb_dirpath.iterdir())])

        intrinsics_path = scene_dirpath / f'CameraIntrinsics{self.resolution_suffix}.csv'
        intrinsics = np.loadtxt(intrinsics_path, delimiter=',').reshape(-1, 3, 3)
        # Assume shared intrinsics between all cameras.
        for intrinsic in intrinsics:
            assert np.all(intrinsic == intrinsics[0])
        fx, fy, cx, cy = intrinsics[0, 0, 0], intrinsics[0, 1, 1], intrinsics[0, 0, 2], intrinsics[0, 1, 2]
        pixtocam = np.linalg.inv(camera_utils.intrinsic_matrix(fx, fy, cx, cy))
        # TODO SNB: check if skew parameters could be included
        # pixtocam = np.linalg.inv(intrinsics[0])

        extrinsics_path = scene_dirpath / f'CameraExtrinsics.csv'
        extrinsics = np.loadtxt(extrinsics_path, delimiter=',').reshape(-1, 4, 4)
        ...
        if self.camera_type == 0 or self.camera_type == 'SIMPLE_PINHOLE':
            params = None
            camtype = camera_utils.ProjectionType.PERSPECTIVE
        elif self.camera_type == 1 or self.camera_type == 'PINHOLE':
            params = None
            camtype = camera_utils.ProjectionType.PERSPECTIVE
        elif self.camera_type == 2 or self.camera_type == 'SIMPLE_RADIAL':
            # TODO SNB: Obtain k1 from the dataset
            params = {k: 0. for k in ['k1', 'k2', 'k3', 'p1', 'p2']}
            params['k1'] = cam.k1
            camtype = camera_utils.ProjectionType.PERSPECTIVE
        elif self.camera_type == 3 or self.camera_type == 'RADIAL':
            # TODO SNB: Obtain k1 and k2 from the dataset
            params = {k: 0. for k in ['k1', 'k2', 'k3', 'p1', 'p2']}
            params['k1'] = cam.k1
            params['k2'] = cam.k2
            camtype = camera_utils.ProjectionType.PERSPECTIVE
        elif self.camera_type == 4 or self.camera_type == 'OPENCV':
            # TODO SNB: Obtain k1, k2, p1 and p2 from the dataset
            params = {k: 0. for k in ['k1', 'k2', 'k3', 'p1', 'p2']}
            params['k1'] = cam.k1
            params['k2'] = cam.k2
            params['p1'] = cam.p1
            params['p2'] = cam.p2
            camtype = camera_utils.ProjectionType.PERSPECTIVE
        elif self.camera_type == 5 or self.camera_type == 'OPENCV_FISHEYE':
            # TODO SNB: Obtain k1, k2, k3 and k4 from the dataset
            params = {k: 0. for k in ['k1', 'k2', 'k3', 'k4']}
            params['k1'] = cam.k1
            params['k2'] = cam.k2
            params['k3'] = cam.k3
            ...
            raise RuntimeError(f'Unknown camera type: {self.camera_type}')

        return frame_names, poses, pixtocam, params, camtype, intrinsics, extrinsics
...
class Dataset(torch.utils.data.Dataset):
  ...
  """
    ...
    def __init__(self,
        ...
        self.scene_dirpath = self.data_dir / f'all/database_data/{config.scene_name}'
        ...
        self.frame_names: List[str] = None
        self.camtoworlds: np.ndarray = None
        self.pixtocams: np.ndarray = None
        self.height: int = None
        self.width: int = None

        # Raw intrinsics and extrinsics are used by the Augmentations loss
        self.intrinsics_raw: np.ndarray = Optional[None]
        self.extrinsics_raw: np.ndarray = Optional[None]
    ...
    def __len__(self):
        if self.split == utils.DataSplit.TRAIN and not self.config.compute_visibility:
            return 1000  # TODO SNB: understand why
        else:
            return self._n_examples
    ...
    """
        ...
        if not self.render_path:
            ...
            if self.alphas is not None:
                batch['alpha'] = self.alphas[cam_idx, pix_y_int, pix_x_int]
            if isinstance(cam_idx, np.int64) and self.frame_names is not None:
                batch['frame_name'] = self.frame_names[cam_idx]
        if self._load_disps:
        ...
        if augmentation_data_needed:
            ...
            common_data = {
                ...
                'intrinsics_all': self.intrinsics_raw,
                'extrinsics_all_w2c_opencv': self.extrinsics_raw,
                'extrinsics_all_c2w_nerf': self.poses,
                'resolution': (self.height, self.width)
            }
        batch_tr = {}
        for k, v in batch.items():
            if isinstance(v, np.ndarray):
                batch_tr[k] = torch.from_numpy(v.copy()).float()
            else:
                batch_tr[k] = v
        if common_data is not None:
            batch_tr['common_data'] = {}
            for k, v in common_data.items():
                if isinstance(v, np.ndarray):
                    batch_tr['common_data'][k] = torch.from_numpy(v.copy()).float()
                else:
                    batch_tr['common_data'][k] = v
        return batch_tr
...
class Blender(Dataset):
    ...
    def _load_renderings(self, config):
        ...
        if self.split == DataSplit.TRAIN:
            train_videos_path = self.data_dir / f'train_test_sets/set{self.config.train_set_num:02}/TrainVideosData.csv'
            train_videos_data = pandas.read_csv(train_videos_path)
            frame_nums = train_videos_data[train_videos_data['scene_name']==self.config.scene_name]['pred_frame_num'].to_numpy()
        elif self.split == DataSplit.TEST:
            test_videos_path = self.data_dir / f'train_test_sets/set{self.config.test_set_num:02}/TestVideosData.csv'
            test_videos_data = pandas.read_csv(test_videos_path)
            frame_nums = test_videos_data[test_videos_data['scene_name']==self.config.scene_name]['pred_frame_num'].to_numpy()
        else:
            raise ValueError(f'Unknown split: {self.split}')
        intrinsics_path = self.data_dir / f'all/database_data/{self.config.scene_name}/{self.split.value.lower()}/CameraIntrinsics.csv'
        extrinsics_path = self.data_dir / f'all/database_data/{self.config.scene_name}/{self.split.value.lower()}/CameraExtrinsics.csv'
        intrinsics = np.loadtxt(intrinsics_path, delimiter=',').reshape(-1, 3, 3)[frame_nums]
        extrinsics = np.loadtxt(extrinsics_path, delimiter=',').reshape(-1, 4, 4)[frame_nums]
        for frame_num in tqdm(frame_nums):
            rgb_path = self.data_dir / f'all/database_data/{self.config.scene_name}/{self.split.value.lower()}/rgb/{frame_num:04d}.png'
            rgb = skimage.io.imread(rgb_path.as_posix()).astype('float32') / 255.
            images.append(rgb)

            if self._load_disps:
                disp_path = self.data_dir / f'all/database_data/{self.config.scene_name}/{self.split.value.lower()}/disp/{frame_num:04d}.tiff'
                disp = skimage.io.imread(disp_path.as_posix()).astype('float32')
                disp_images.append(disp)

            if self._load_normals:
                normal_path = self.data_dir / f'all/database_data/{self.config.scene_name}/{self.split.value.lower()}/normal/{frame_num:04d}.png'
                normal = skimage.io.imread(normal_path.as_posix()).astype('float32') / 255.
        ...
        permuter = np.eye(4)
        permuter[1:3] *= -1
        camtoworlds = np.linalg.inv(np.linalg.inv(permuter)[None] @ extrinsics @ permuter[None])
        pixtocams = np.linalg.inv(intrinsics)
        rgb, alpha = self.images[..., :3], self.images[..., -1:]
        self.images = rgb * alpha + (1. - alpha)  # Use a white background.
        self.alphas = alpha
        self.height, self.width = self.images.shape[1:3]
        self.poses = camtoworlds
        self.camtoworlds = camtoworlds
        self.pixtocams = pixtocams[0]
        self.translation_scale = 1
        if self.split == DataSplit.TRAIN:
            # SNB: frame_nums used during evaluation to save frames. So, add 200 to avoid overwriting test frames.
            self.frame_names = [f'{frame_num + 200:04}' for frame_num in frame_nums]
        elif self.split == DataSplit.TEST:
            self.frame_names = [f'{frame_num:04}' for frame_num in frame_nums]
        elif self.split == DataSplit.VALIDATE:
            self.frame_names = [f'{frame_num + 300:04}' for frame_num in frame_nums]
        self.extrinsics_raw = extrinsics
        self.intrinsics_raw = intrinsics
        return
...
class LLFF(Dataset):
    ...
    def _load_renderings(self, config):
        ...
        if config.factor > 0 and not (config.rawnerf_mode and
            ...
            resolution_suffix = f'_down{config.factor}'
        else:
            factor = 1
            resolution_suffix = ''

        # Load poses.
        frame_names, poses, pixtocam, distortion_params, camtype, intrinsics_raw, extrinsics_raw = NeRFSceneManager(self.data_dir, self.config.scene_name, resolution_suffix).process()

        # Previous NeRF results were generated with images sorted by filename,
        # use this flag to ensure metrics are reported on the same test set.
        # SNB: The images are sorted when extracting the dataset. So, no need of sorting here
        # inds = np.argsort(frame_names)
        # frame_names = [frame_names[i] for i in inds]
        # poses = poses[inds]
        ...
        # SNB: No need to scale the inverse intrinsics matrix by the image downsampling factor. Downsampled intrinsics
        # already loaded.
        ...
        if config.forward_facing:
            ...
            translation_scale = 1. / (bounds.min() * .75)
            poses[:, :3, 3] *= translation_scale
            self.colmap_to_world_transform = np.diag([translation_scale] * 3 + [1])
            bounds *= translation_scale
            # Recenter poses.
            ...
            poses, transform, translation_scale = camera_utils.transform_poses_pca(poses)
            self.colmap_to_world_transform = transform
            self.translation_scale = translation_scale
            if config.render_spline_keyframes is not None:
                rets = camera_utils.create_render_spline_path(config, frame_names,
                ...
                # SNB: Instead of generating poses here, load the pre-created poses
                if self.split == DataSplit.TRAIN:
                    set_num = config.train_set_num
                elif self.split == DataSplit.TEST:
                    set_num = config.test_set_num
                else:
                    raise ValueError(f'Unknown split: {self.split}')
                render_poses_path = self.data_dir / f'train_test_sets/set{set_num:02}/video_poses{self.config.video_poses_num:02}/{self.config.scene_name}.csv'
                render_poses = np.loadtxt(render_poses_path, delimiter=',').reshape(-1, 4, 4)[1:]
                video_poses_temporal_downsampling_factor = render_poses.shape[0] // config.video_poses_num_frames
                render_poses = render_poses[::video_poses_temporal_downsampling_factor]
                render_poses_std = np.linalg.inv(render_poses) @ np.diag([1, -1, -1, 1])
                render_poses_transformed = transform @ render_poses_std
                render_poses_transformed[:, :3, 3] *= translation_scale
                self.render_poses = render_poses_transformed[:, :3]

        # Select the split.
        if self.split == DataSplit.TRAIN:
            train_videos_path = self.data_dir / f'train_test_sets/set{self.config.train_set_num:02}/TrainVideosData.csv'
            train_videos_data = pandas.read_csv(train_videos_path)
            indices = train_videos_data[train_videos_data['scene_name']==self.config.scene_name]['pred_frame_num'].to_numpy()
        elif self.split == DataSplit.TEST:
            test_videos_path = self.data_dir / f'train_test_sets/set{self.config.test_set_num:02}/TestVideosData.csv'
            test_videos_data = pandas.read_csv(test_videos_path)
            indices = test_videos_data[test_videos_data['scene_name']==self.config.scene_name]['pred_frame_num'].to_numpy()
        else:
            raise ValueError(f'Unknown split: {self.split}')
        ...
        #     local_indices = [i for i in range(len(frame_names)) if (i + self.global_rank) % self.world_size == 0]
        #     frame_names = [frame_names[i] for i in local_indices]
        ...
        if config.rawnerf_mode:
            ...
            # Load images.
            image_paths = [self.scene_dirpath / f'rgb{resolution_suffix}/{frame_name}.png' for frame_name in frame_names]
            ...
            jpeg_paths = [self.scene_dirpath / f'rgb/{frame_name}.jpg' for frame_name in frame_names]
        ...
        self.poses = poses
        self.frame_names = frame_names  # This is used at test-time to obtain image name
        self.images = images
        self.camtoworlds = self.render_poses if config.render_path else poses
        self.height, self.width = images.shape[1:3]

        # Raw intrinsics and extrinsics are used for augmentation loss
        extrinsics_raw[:, :3, 3] *= translation_scale
        self.intrinsics_raw = intrinsics_raw[indices]
        self.extrinsics_raw = extrinsics_raw[indices]
        return
