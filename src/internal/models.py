...
class Model(nn.Module):
    ...
    def __init__(self, config=None, **kwargs):
        ...
        elif self.model_name == 'augmentation01':
            self.nerf_mlp = NerfMLPAugmentation01(num_glo_features=self.num_glo_features,
                                                  num_glo_embeddings=self.num_glo_embeddings)
        elif self.model_name == 'augmentation02':
            self.nerf_mlp = NerfMLPAugmentation02(num_glo_features=self.num_glo_features,
                                                  num_glo_embeddings=self.num_glo_embeddings)
        else:
        ...
        if self.single_mlp:
            ...
            if self.model_name == 'main':
                ...
                self.prop_mlp = PropMLPAugmentation01()
            elif self.model_name == 'augmentation02':
                self.prop_mlp = PropMLPAugmentation02()
            else:
                raise ValueError(f'Unknown model name: {self.name}')
        else:
            for i in range(self.num_levels - 1):
                if self.model_name == 'main':
                    self.register_module(f'prop_mlp_{i}', PropMLP(grid_desired_resolution=self.prop_desired_grid_size[i]))
                elif self.model_name == 'augmentation01':
                    self.register_module(f'prop_mlp_{i}', PropMLPAugmentation01(grid_desired_resolution=self.prop_desired_grid_size[i]))
                elif self.model_name == 'augmentation02':
                    self.register_module(f'prop_mlp_{i}', PropMLPAugmentation02(grid_desired_resolution=self.prop_desired_grid_size[i]))
                else:
                    raise ValueError(f'Unknown model name: {self.name}')
    ...
    """
        ...
        if self.near_anneal_rate is not None:
            init_s_near = np.clip(1 - train_frac / self.near_anneal_rate, 0, self.near_anneal_init)
        elif self.near_depth_s > 0:
            init_s_near = self.near_depth_s
        else:
            init_s_near = 0.
        init_s_far = 1.
        sdist = torch.cat([
...
class MLP(nn.Module):
    ...
    grid_desired_resolution: int = 8192
    ...
    def __init__(self, **kwargs):
        ...
        self.grid_num_levels = int(
            np.log(self.grid_desired_resolution / self.grid_base_resolution) / np.log(self.grid_level_interval)) + 1
        self.encoder = GridEncoder(input_dim=3,
                                   ...
                                   desired_resolution=self.grid_desired_resolution,
...
@gin.configurable
class ModelAugmentation01(Model):
    pass


@gin.configurable
class ModelAugmentation02(Model):
    pass


@gin.configurable
class NerfMLPAugmentation01(MLP):
    pass


@gin.configurable
class NerfMLPAugmentation02(MLP):
    pass


@gin.configurable
class PropMLPAugmentation01(MLP):
    pass


@gin.configurable
class PropMLPAugmentation02(MLP):
    pass
