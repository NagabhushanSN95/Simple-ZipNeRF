...
# Modified from R21/VSR012/ConfigsCreator.py
...
def save_train_configs(args):
    ...
    for scene_name in args.scene_names:
        ...
        # For indoor scenes, reduce factor by 2
        if scene_name in ['bonsai', 'counter', 'kitchen', 'room']:
            config_matcher_line = 'Config.factor *= *(\\d)\\n'
            match = re.search(config_matcher_line, ''.join(scene_configs))
            if match is not None:
                factor = int(match.group(1))
                if factor >= 2:
                    factor = factor // 2
                    # We need to overwrite the existing configs line. So, remove it.
                    config_line_index = scene_configs.index(match.group(0))
                    scene_configs.remove(match.group(0))
                    config_line = f'Config.factor = {factor}\n'
                    scene_configs.insert(config_line_index, config_line)
            else:
                raise RuntimeError(f'factor not configured.')
