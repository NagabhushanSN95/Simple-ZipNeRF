...
def save_rgb_video(path: Path, frames_dirpath: Path, framerate: int):
    cmd = f"ffmpeg -y -framerate {framerate} -pattern_type glob -i '{frames_dirpath.as_posix()}/*.png' -frames:v 200 -c:v libx264 -pix_fmt yuv420p {path.as_posix()}"
    os.system(cmd)
    return
...
def save_depth_video(path: Path, depths_dirpath: Path, framerate: int):
    ...
    depths = []
    for depth_path in tqdm(sorted(depths_dirpath.glob('*.npy'))[:200]):
...
def save_distance_video(path: Path, distance_dirpath: Path, framerate: int, render_dist_percentile: float):
    ...
    for distance_path in tqdm(sorted(distance_dirpath.glob('*.tiff'))[:200]):
...
def save_acc_video(path: Path, acc_dirpath: Path, framerate: int):
    accs = []
    for acc_path in tqdm(sorted(acc_dirpath.glob('*.tiff'))[:200]):
...
def generate_videos(test_dirpath: Path, scene_names: List[str], framerate: int, render_dist_percentile: float):
    for scene_name in scene_names:
        scene_dirpath = test_dirpath / scene_name
        # Create RGB video
        pred_frames_dirpath = scene_dirpath / 'predicted_frames'
        pred_video_path = scene_dirpath / 'PredictedVideo.mp4'
        save_rgb_video(pred_video_path, pred_frames_dirpath, framerate)

        # Create depth video
        pred_depths_dirpath = scene_dirpath / 'predicted_depths'
        pred_depth_video_path = scene_dirpath / 'PredictedDepth.mp4'
        save_depth_video(pred_depth_video_path, pred_depths_dirpath, framerate)

        # Create distance mean videos
        pred_distance_mean_dirpath = scene_dirpath / 'predicted_distance_mean'
        pred_distance_mean_video_path = scene_dirpath / 'PredictedDistanceMean.mp4'
        ...
        pred_distance_median_dirpath = scene_dirpath / 'predicted_distance_median'
        pred_distance_median_video_path = scene_dirpath / 'PredictedDistanceMedian.mp4'
        save_distance_video(pred_distance_median_video_path, pred_distance_median_dirpath, framerate, render_dist_percentile)

        # Create acc videos
        pred_acc_dirpath = scene_dirpath / 'predicted_acc'
        pred_acc_video_path = scene_dirpath / 'PredictedAcc.mp4'
