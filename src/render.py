...
def main(unused_argv):
    ...
    config.exp_path = Path(f'../runs/testing/test{config.test_num:04}/{config.scene_name}_video{config.video_poses_num:02}')
    ...
    logging.basicConfig(
        ...
        handlers=[logging.StreamHandler(sys.stdout),
                  logging.FileHandler((config.logs_dir / 'log_render.txt').as_posix())],
        level=logging.INFO,
    )
    ...
    for idx in tqdm(range(dataset.size)):
        ...
        batch = next(dataiter)
        curr_file = path_fn('predicted_frames', idx, 'png')
        ...
        if accelerator.is_main_process:  # Only record via host 0.
            ...
            rendering['depth'] = postprocess_fn(rendering['depth'])
            rendering = tree_map(lambda x: x.detach().cpu().numpy() if x is not None else None, rendering)
            utils.save_img_u8(rendering['rgb'], path_fn('predicted_frames', idx, 'png'))
            utils.save_depth(rendering['depth'],
                             path_fn(f'predicted_depths', idx, 'npy'), as_png=True)
            if 'normals' in rendering:
                utils.save_img_u8(rendering['normals'] / 2. + 0.5,
                                  path_fn('predicted_normals', idx, 'png'))
            utils.save_img_f32(rendering['distance_mean'],
                               path_fn(f'predicted_distance_mean', idx, 'tiff'))
            utils.save_img_f32(rendering['distance_median'],
                               path_fn(f'predicted_distance_median', idx, 'tiff'))
            utils.save_img_f32(rendering['acc'], path_fn(f'predicted_acc', idx, 'tiff'))
    num_files = len(sorted(config.exp_path.glob('predicted_acc/*.tiff')))
