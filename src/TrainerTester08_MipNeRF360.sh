...
do
    ...
    python utils/ConfigsCreator08_MipNeRF360.py --configs-path $1 --scene-names $scene_name --mode train  --train-set-num $train_set_num
    ...
    python utils/ConfigsCreator08_MipNeRF360.py --configs-path $1 --scene-names $scene_name --mode test  --test-num $test_num --test-set-num $test_set_num --train-set-num $train_set_num
    ...
    # Run QA
    conda run --live-stream -n $qa_env_name bash -c "python $qa_filepath --demo_function_name demo2 --pred_videos_dirpath $test_dirpath --database_dirpath $database_dirpath --gt_depth_dirpath $gt_depth_dirpath --frames_datapath $test_frames_datapath --pred_frames_dirname predicted_frames --pred_depths_dirname predicted_depths --mask_folder_name $qa_masks_dirname --resolution_suffix _down4"

    # Call rendering
    python render.py --gin_configs=$scene_test_configs_path --gin_bindings="Config.render_path = True" --gin_bindings="Config.batch_size = $batch_size" --gin_bindings="Config.render_chunk_size = $render_chunk_size"

    # Create videos
    conda run --live-stream -n $video_env_name bash -c "python utils/VideosCreator08_MipNeRF360.py --test-dirpath $test_dirpath --scene-names $scene_name --framerate $framerate"
done
